...
An additional consideration is the space usage of device data. You can limit data allocated to the oplog with the oplogSize paramater that can be passed as an argument to the mongodb daemon. Furthermore, when inserting or updating documents, in each document, node-thing will also set the value of `lastModified` to be the date at which that document was inserted or last updated. You can use this property to set a time-to-live for each collection after which documents will be deleted by following the documention [here](http://docs.mongodb.org/manual/tutorial/expire-data/).
